-- Prompt 22: persist built-in spread identity without creating spread tables.
alter table public.readings
  add column spread_id text,
  add constraint readings_spread_id_check
    check (spread_id is null or spread_id in ('single-card','three-cards','situation','open'));

alter table public.reading_cards
  add column spread_position_id text;

comment on column public.readings.spread_id is 'Nullable built-in spread identifier; null preserves legacy readings.';
comment on column public.reading_cards.spread_position_id is 'Nullable stable built-in spread position identifier.';

create index readings_user_spread_idx on public.readings(user_id,spread_id) where spread_id is not null;

drop function public.create_reading_with_cards(uuid,uuid,text,timestamptz,text,text,text,jsonb);
drop function public.update_reading_with_cards(uuid,uuid,uuid,text,timestamptz,text,text,text,jsonb);
drop function public.validate_reading_cards(jsonb,text);

create function public.validate_reading_cards(p_cards jsonb,p_status text,p_spread_id text)
...
create function public.create_reading_with_cards(p_topic_id uuid,p_question_template_id uuid,p_temporary_question text,p_reading_at timestamptz,p_reading_timezone text,p_interpretation text,p_status text,p_spread_id text,p_cards jsonb)
...
begin
  ...
  insert into public.readings(user_id,topic_id,question_template_id,question_text_snapshot,reading_at,reading_timezone,interpretation,status,spread_id)
  values(v_user,p_topic_id,p_question_template_id,v_question,p_reading_at,p_reading_timezone,p_interpretation,p_status,p_spread_id) returning id into v_id;
...
create function public.update_reading_with_cards(p_reading_id uuid,p_topic_id uuid,p_question_template_id uuid,p_temporary_question text,p_reading_at timestamptz,p_reading_timezone text,p_interpretation text,p_status text,p_spread_id text,p_cards jsonb)
returns uuid language plpgsql security invoker set search_path=pg_catalog,public,auth as $$
...
begin
  ...
  if p_question_template_id is not null then select question_text into v_question from public.question_templates where id=p_question_template_id and topic_id=p_topic_id and user_id=v_user; if v_question is null then raise exception 'template unavailable' using errcode='42501'; end if; else v_question:=nullif(btrim(p_temporary_question),''); end if;
  perform public.validate_reading_cards(p_cards,p_status,p_spread_id);
  update public.readings set topic_id=p_topic_id,question_template_id=p_question_template_id,question_text_snapshot=v_question,reading_at=p_reading_at,reading_timezone=p_reading_timezone,interpretation=p_interpretation,status=p_status,spread_id=p_spread_id where id=p_reading_id and user_id=v_user;
...
revoke all on function public.validate_reading_cards(jsonb,text,text) from public,anon;
revoke all on function public.create_reading_with_cards(uuid,uuid,text,timestamptz,text,text,text,text,jsonb) from public,anon;
revoke all on function public.update_reading_with_cards(uuid,uuid,uuid,text,timestamptz,text,text,text,text,jsonb) from public,anon;
grant execute on function public.validate_reading_cards(jsonb,text,text) to authenticated;
grant execute on function public.create_reading_with_cards(uuid,uuid,text,timestamptz,text,text,text,text,jsonb) to authenticated;
grant execute on function public.update_reading_with_cards(uuid,uuid,uuid,text,timestamptz,text,text,text,text,jsonb) to authenticated;
