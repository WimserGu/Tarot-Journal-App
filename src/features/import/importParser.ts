...
import { findTarotCardByName } from '../../domain/readingUtils';
...
function parseCard(line: string): ImportCardCandidate {
  ...
  const card = findTarotCardByName(match[1]!, tarotCards);
...
export const IMPORT_AI_PROMPT = `请把下面的塔罗记录整理为严格格式。\n\n不要解释。\n不要分析。\n不要总结。\n不要分类。\n不要补充缺失信息。\n不要遗漏任何一次抽牌。\n不要改变问题原意。\n保持原始顺序。\n\n每条记录必须输出为：\n[Reading]\nDate: YYYY-MM-DD\nTopic:\nQuestion:\nCards:\n- 牌名 | upright\n- 牌名 | reversed\n- 牌名 | reversed | left\n- 牌名 | reversed | right\nNotes:\n\n规则：没有日期、Topic 或 Notes 时留空；牌名使用标准中文塔罗牌名，其中使用“愚人”“隐士”“首牌”“侍从”“王后”“国王”；原文只有“逆位”时输出 reversed；原文明示“左旋”时输出 reversed | left；原文明示“右旋”时输出 reversed | right；不得根据牌义猜测左右；只输出整理结果。\n\n示例：\n[Reading]\nDate: 2026-07-13\nTopic: 关系\nQuestion: 她现在想对我说什么？\nCards:\n- 星币国王 | upright\nNotes:`;
